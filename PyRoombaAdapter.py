...
Written Atsushi Sakai, 2019

Adapted by Andy Grace, 2023
...
class PyRoombaAdapter:
    ...
    SENSOR = {
        ...
        "Light Bumper Left" : (46,2,False),
        "Light Bumper Left Front" : (47,2,False),
        "Light Bumper Left Center" : (48,2,False),
        "Light Bumper Right Center" : (49,2,False),
        "Light Bumper Right Front" : (50,2,False),
        "Light Bumper Right" : (51,2,False),
    }
    ...
    def request_bump_and_wheels(self):
        """
        Requests the state of the bump sensor and the wheel lift sensors

        :returns: Array of bump sensor and wheel sensor data. Index 0 is left bump, index 1 is right bump, index 2 is left wheel drop, index 3 is right wheel drop
        """
        data = self._request_sensor("Bump Sensor")
        return [(data & 2) >> 1, data & 1, (data & 8) >> 3, (data & 4) >> 2]
    
    def request_buttons(self):
        """
        Requests the state of the buttons

        I mainly just programmed this to figure out the bit order because the bump and wheels method wasn't working great.

        The end should be dock, spot, clean

        :returns: Array of bump sensor and wheel sensor data. Index 0 is left bump, index 1 is right bump, index 2 is left wheel drop, index 3 is right wheel drop
        """
        data = self._request_sensor("Buttons")
        return [(data & 128) >> 7, data & 64 >> 6, (data & 32) >> 5, (data & 16) >> 4, (data & 8) >> 3,(data & 4) >> 2,(data & 2) >> 1, data & 1]

    def request_light_bumper_summary(self):
        """
        Requests the state of the light bumper sensors 

        :returns: Array of bump sensor and wheel sensor data. The array goes clockwise, starting from left.
        """
        data = self._request_sensor("Light Bumper")
        return [
            data & 1,
            (data & 2) >> 1,
            (data & 4) >> 2,
